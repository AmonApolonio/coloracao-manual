...
const getTemperatureCategory = (value: number, stepKey?: string): string => {
...
// Helper function to get ComparisonField from comparison name
const getComparisonFieldByName = (name: string): ComparisonField | null => {
  for (const [field, fieldName] of Object.entries(COMPARISON_FIELD_NAMES)) {
    if (name === fieldName) {
      return field as ComparisonField
    }
  }
  return null
}
...
export const convertDBToUI = (
  ...
  Object.keys(extractedColors).forEach(field => {
    ...
    temperaturaUI[field] = {
      ...
      temperatureCategory: value != null ? getTemperatureCategory(value, 'temperatura') : '',
    }
  })
  ...
  Object.keys(extractedColors).forEach(field => {
    ...
    intensidadeUI[field] = {
      hexColor: extractedColors[field] || '#000000',
      temperature: value ?? null,
      temperatureCategory: value != null ? getTemperatureCategory(value, 'intensidade') : '',
    }
  })
  ...
  const profundidadeUI: ProfundidadeComparisonUI[] = [
    {
      ...
      category: dbData.profundidade?.['iris_vs_pele'] != null 
        ? getTemperatureCategory(dbData.profundidade['iris_vs_pele'], 'profundidade')
        : '',
    },
    {
      ...
      category: dbData.profundidade?.['cavidade_ocular_vs_pele'] != null
        ? getTemperatureCategory(dbData.profundidade['cavidade_ocular_vs_pele'], 'profundidade')
        : '',
    },
    {
      ...
      category: dbData.profundidade?.['cabelo_vs_pele'] != null
        ? getTemperatureCategory(dbData.profundidade['cabelo_vs_pele'], 'profundidade')
        : '',
    },
    {
      ...
      category: dbData.profundidade?.['contorno_boca_vs_boca'] != null
        ? getTemperatureCategory(dbData.profundidade['contorno_boca_vs_boca'], 'profundidade')
        : '',
    },
  ]
